...
l,= ax.plot((En[:-1]+En[1:])/2, 1/(test.interp(rho0, T0)*rho0)) # dens T
...
def update(val):
    rho= 10**(rhoSl.val)
    T= 10**(TSl.val)
    l.set_ydata(1/(test.interp(rho, T)*rho))
...
def new_opac(label):
    ...
    ax.set_ylabel('Opacity ' + label +' (cm)')
    update(1)
...
#    test= op.OplGrid(Kr['dens'], Kr['temp'], Kr['groups'], lambda jd, jt: Kr['opp_mg'][jd, jt, :])
    l.set_data((En[:-1]+En[1:])/2, 1/(En[:-1]*rho0))
